 ...
 * `window.__ModuleLoader__.load` 惰性 CJS factory。
 *
 * ── DSH 0.1.5 适配（重要）─────────────────────────────────────────────────
 * 运行时模块表（`@deepseek-ai/dsh-client-web` 的 PLATFORM_MODULES / seed.ts）
 * 只共享这些"平台单例"：
 *   react, react/jsx-runtime, react-dom, react-dom/client, @deepseek-ai/cordis,
 *   @deepseek-ai/dsh-client-store, @deepseek-ai/dsh-client-ui-slots,
 *   @deepseek-ai/dsh-client-ui-primitives, @deepseek-ai/dsh-client-ui-dockkit
 * 旧的 `@deepseek-ai/dsh-client-runtime/client` 已不存在 → 改为直接从
 * `@deepseek-ai/dsh-client-store` 取 `createSnapshotStore`（同一个引擎：
 * zustand vanilla + immer, 契约不变：getSnapshot / subscribe / set / update）。
 * 任何 require 打到模块表之外都会抛
 * "require(...) missed the module table"，整张卡片随之消失。
 *
 ...
 * - scope 不可用（插件禁用/未安装/该 namespace 未被 Host 服务）自动隐藏对应
 *   小节, 底部给出"组合配置"说明（argp / sgme / vault / mcp-lens /
 *   context-doctor 无 settings 命名空间, 参数在 cordis.patch.yml 的对应行
 *   config 中调整）。
 *
 * 注意（0.1.5 的 tab-store 契约）：`settings.plugin.item` 只渲染
 * "Host 已服务的 namespace ∩ 已注册的卡片 key" 的交集——因此 key 必须等于
 * 某个真实注册的 namespace（本卡片用 `brain-compaction`），Host 半区注册失败
 * 时整卡不会出现（不是渲染 bug）。
 */
function brainCompactionClientFactory(require) {
  ...
  // 0.1.5：快照 store 是平台 seed 词, 直接可用（不再经 dsh-client-runtime）。
  ...
  // ── plugin body ───────────────────────────────────────────────────────
  // 0.1.5：`slots` / `locale` / `settingsScope` 三个服务由 boot graph 提供。
  // `connection` 已无用（本卡不读连接状态），移除以免依赖一个缺席的服务
  // 让整个 browser fiber 卡在 pending。
