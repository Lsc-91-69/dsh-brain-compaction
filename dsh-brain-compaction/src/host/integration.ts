...
export function compactionHistory(session: unknown, limit = 3): {
  ...
  const events = (session as { events?: unknown } | null)?.events
  const hits: Array<{ seq?: number; data: Record<string, string | number | boolean> }> = []
  if (Array.isArray(events)) {
    for (const e of events) {
      const evt = e as { type?: unknown; seq?: unknown; data?: unknown }
...
function countAll(events: unknown[] | undefined): number {
